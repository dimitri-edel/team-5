...
def test_dislike_profile():
    ...
    # Confirm that the dislike object exists in the database
    dislike = Dislikes.objects.get(user=tester_profile, dislikes=target_profile)
    assert dislike.user == tester_profile
    assert dislike.dislikes == target_profile
