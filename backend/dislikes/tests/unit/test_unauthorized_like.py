...
def test_unauthorized_like():
    ...
    # Step 4: Have the tester user like the profile of the target user
    like_url = reverse('likeendpoint', kwargs={'pk': target_profile.pk})  # /like/<int:pk>/
    like_response = client.post(like_url, format='json')
    
    # Step 5: Confirm that a like object has been created. Response status is 201 CREATED
    ...
    # Step 8: Have the target user like their own profile
    self_like_url = reverse('likeendpoint', kwargs={'pk': target_profile.pk})  # /like/<int:pk>/
    self_like_response = client.post(self_like_url, format='json')
    
    # Step 9: Confirm that the access has been denied. Response status is 403. And Response message is "You cannot like your own profile"
    assert self_like_response.status_code == status.HTTP_403_FORBIDDEN
    assert self_like_response.data['message'] == "You cannot like your own profile"
