...
def test_list_profiles():
    ...
    # Step 12: Have tester user request a list of matched profiles
    matched_profiles_url = reverse('matched-profiles')
    matched_profiles_response = client.get(matched_profiles_url, format='json')
    
    print(matched_profiles_response.data)
    # Step 13: Confirm that there are 2 profiles in the response
    assert matched_profiles_response.status_code == status.HTTP_200_OK
    matched_profiles = matched_profiles_response.data
    assert len(matched_profiles) == 2
    
    # Step 14: Confirm that the profiles in the response are the ones of user1 and user2
    matched_profile_ids = [profile['profile_id'] for profile in matched_profiles]
